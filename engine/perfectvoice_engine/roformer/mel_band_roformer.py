...
from perfectvoice_engine.roformer.attend import Attend
...
class MelBandRoformer(Module):
    ...
    def forward(
        ...
        if stft_repr_expanded_stems.is_complex():
            masks_real = torch.zeros_like(stft_repr_expanded_stems.real).scatter_add_(2, scatter_indices, masks.real)
            masks_imag = torch.zeros_like(stft_repr_expanded_stems.imag).scatter_add_(2, scatter_indices, masks.imag)
            masks_summed = torch.complex(masks_real, masks_imag)
        else:
